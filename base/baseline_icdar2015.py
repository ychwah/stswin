...
launcher = 'none'
resume = False
work_dir = './work_dirs/v3_icdar2015'

#  Common meta pack (prevents 'flip' key error) 
pack_meta = dict(
...
# Train pipeline (added FixInvalidPolygon for stability; keeps your v3 ordering)
train_pipeline = [
...
# Val/Test pipeline (kept same; add FixInvalidPolygon is harmless)
test_pipeline = [
    dict(type='LoadImageFromFile'),
...
# Test dataset (used for both val & test)
test_dataset = dict(
    type='mmocr.OCRDataset',
    data_root=data_root,
...
model = dict(
    ...
    det_head=dict(
        type='mmocr.DBHead',
        in_channels=512,  # keep as in your working v3.py
        module_loss=dict(type='mmocr.DBModuleLoss'),
        postprocessor=dict(
            type='mmocr.DBPostprocessor',
            text_repr_type='quad',  # IC15 is box-like
            mask_thr=0.3,
            min_text_score=0.3,
            min_text_width=5,
            unclip_ratio=1.5,
...
# Optim / Schedulers (same as your v3.py)
optim_wrapper = dict(
